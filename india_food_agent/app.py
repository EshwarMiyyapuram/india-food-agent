...
if "feedbacks"    not in st.session_state: st.session_state.feedbacks    = _load_feedbacks()
...
#  TABS
# ══════════════════════════════════════════
tab1, tab2, tab3, tab4 = st.tabs(["📊  Trend Analysis", "🍽  Weekend Specials", "📋  Weekly Report", "⭐  Feedback"])
...
with tab4:
    ...
    <div style="display:flex;gap:16px;margin-bottom:24px">
      <div style="flex:1;background:var(--card-bg);border:1px solid var(--border);border-radius:14px;padding:18px 20px;text-align:center">
        <div style="font-family:'DM Mono',monospace;font-size:28px;font-weight:700;color:var(--spice);line-height:1">{len(feedbacks)}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:4px;font-weight:500">Total Reviews</div>
      </div>
      <div style="flex:1;background:var(--card-bg);border:1px solid var(--border);border-radius:14px;padding:18px 20px;text-align:center">
        <div style="font-family:'DM Mono',monospace;font-size:28px;font-weight:700;color:var(--spice);line-height:1">{"⭐ " + str(avg_rating) if feedbacks else "—"}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:4px;font-weight:500">Avg Rating</div>
      </div>
      <div style="flex:1;background:var(--card-bg);border:1px solid var(--border);border-radius:14px;padding:18px 20px;text-align:center">
        <div style="font-family:'DM Mono',monospace;font-size:28px;font-weight:700;color:var(--spice);line-height:1">{pos_count}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:4px;font-weight:500">Positive (4–5 ★)</div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    with left_col:
        ...
        st.markdown("<p style='font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--spice);margin:14px 0 6px'>⭐ Rating</p>", unsafe_allow_html=True)
        ...
        st.markdown("<p style='font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--spice);margin:14px 0 6px'>📂 Category</p>", unsafe_allow_html=True)
        fb_category = st.selectbox("Category", [
            "Overall App", "Trend Analysis", "Weekend Specials Quality",
            "Weekly Report", "UI & Design", "Other Suggestion",
        ], label_visibility="collapsed", key="fb_cat")

        st.markdown("<p style='font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--spice);margin:14px 0 6px'>💬 Your Feedback</p>", unsafe_allow_html=True)
        ...
        if st.button("✦ Submit Review", type="primary", use_container_width=True, key="fb_submit"):
            if not fb_comment.strip():
                ...
                updated = [entry] + st.session_state.feedbacks
                st.session_state.feedbacks = updated
                _save_feedbacks(updated)
    ...
    with right_col:
        ...
        if not feedbacks:
            ...
            <div class="empty-state" style="padding:40px">
              <div class="empty-icon">💬</div>
              <div class="empty-title" style="font-size:18px">No Reviews Yet</div>
              <div class="empty-sub">Be the first to leave a review!</div>
            </div>""", unsafe_allow_html=True)
        else:
            # Filter by rating
            f_col, _ = st.columns([1, 2])
            with f_col:
                filter_opt = st.selectbox("Filter", ["All ⭐", "5 ★", "4 ★", "3 ★", "2 ★", "1 ★"],
                                          label_visibility="collapsed", key="fb_filter")
            ...
            for fb in shown:
                ...
                <div style="background:var(--card-bg);border:1px solid var(--border);border-radius:16px;
                            padding:18px 20px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.04)">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
                    <div>
                      <div style="font-size:15px;font-weight:700;color:var(--ink)">{fb.get("name","Anonymous")}</div>
                      <div style="font-size:11px;color:var(--muted);margin-top:2px">📍 {fb.get("city","")} &nbsp;·&nbsp; {fb.get("added_at","")}</div>
                    </div>
                    <div style="text-align:right">
                      ...
                      <span style="display:inline-block;font-size:10px;font-weight:700;letter-spacing:0.08em;
                                   ...
                                   border:1px solid rgba(196,65,26,0.2);margin-top:5px">{fb.get("category","")}</span>
                    </div>
                  </div>
                  <div style="font-size:13px;color:#4A3F37;line-height:1.7">"{fb.get("comment","")}"</div>
                </div>""", unsafe_allow_html=True)

                # Delete button
                del_col, _ = st.columns([1, 5])
                with del_col:
                    if st.button("🗑 Delete", key=f"del_{fb['id']}", help="Delete this review"):
                        st.session_state.feedbacks = [f for f in st.session_state.feedbacks if f["id"] != fb["id"]]
                        _save_feedbacks(st.session_state.feedbacks)
                        st.rerun()

            # Export
            st.markdown("<br>", unsafe_allow_html=True)
            df_fb = pd.DataFrame([{
                "Name": f.get("name",""), "City": f.get("city",""),
                "Rating": f.get("rating",""), "Category": f.get("category",""),
                "Comment": f.get("comment",""), "Date": f.get("added_at",""),
            } for f in feedbacks])
            st.download_button("⬇ Export Reviews CSV", df_fb.to_csv(index=False),
